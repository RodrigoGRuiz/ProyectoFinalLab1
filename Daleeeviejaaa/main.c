...
void agregarFavorito(char nombre[])
{
    ...
    if(archivo)
    {
        while(fread(&cliente,sizeof(stUsuario),1,archivo)>0)
        {
            if(strcmpi(nombre,cliente.nombre)==0)
            {
                int fav = 0;

                printf("Ingrese Numero ID Cancion: \n");
                fflush(stdin);
                scanf("%d",&fav);
                int valor =existenciaCancion(fav);
                if(valor == 1)
                {
                    fseek(&cliente,sizeof(cliente)*-1,SEEK_CUR);
                    cliente.cancionesFavoritas[cliente.contadorValidos] = fav;
                    printf("%d",cliente.cancionesFavoritas[0]);
                    cliente.contadorValidos++;
                    printf("%d",cliente.contadorValidos);
                    system("pause");
                    fwrite(&cliente,sizeof(stUsuario),1,archivo);
...
//void muestraListaFavoritos(char nombre[])
//{
//    stUsuario usuario;
//    stCanciones cancion;
//    int i = 0;
//    FILE*archivoUsuario;
//    archivoUsuario = fopen(usuarios, "r+b");
//    if(archivoUsuario)
//    {
//        while(fread(&usuario,sizeof(stUsuario),1,archivoUsuario)>0)
//        {
//            if(strcmpi(nombre,usuario.nombre))
//            {
//                for(i; i< usuario.contadorValidos; i++)
//                {
//
//                    printf("ähi va ahi va");
//
//                }
//            }
//        }
